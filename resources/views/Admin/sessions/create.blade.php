...
@section('title', __('messages.create_session'))

@section('content')
<div class="container px-4 py-8 mx-auto">
    <h1 class="mb-6 text-3xl font-bold">{{ __('messages.create_new_session') }}</h1>
        ...
        <form action="{{ route('admin.sessions.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <option value="booked" {{ old('status') == 'booked' ? 'selected' : '' }}>{{ __('messages.booked') }}</option>
